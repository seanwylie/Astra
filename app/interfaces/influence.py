...
def _local_mind_file_path():
    """Resolved path to the local mind file, if configured."""
...
def save_mind(mind_data, force=False):
    ...
    # Optional S3 backup — skipped when local-only
    from app.config.loader import s3_sync_enabled
    if not s3_sync_enabled():
        logger.debug("[save_mind] S3 sync disabled; skipping remote backup")
    else:
        try:
            import boto3
...
def load_mind():
    ...
    # Only discard a local copy when S3 is the source of truth.
    if s3_sync_enabled() and local_mind_file and os.path.exists(local_mind_file):
    ...
    try:
        ...
        mind_data = {}
        for key, value in (mind_data_dict or {}).items():
            mind_data[key] = value
        ...
        empty = not any(
            mind_data.get(k) for k in ("self_reflections", "self_questions", "stored_knowledge")
        )
        if empty and local_mind_file and os.path.exists(local_mind_file):
            with open(local_mind_file, encoding="utf-8") as fh:
                seeded = json.load(fh)
            if isinstance(seeded, dict):
                mind_data = seeded
                logger.debug("📝 [load_mind] Seeded mind from local fixture %s", local_mind_file)
