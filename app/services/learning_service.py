...
class LearningService:
    ...
    def learn_from_text(self, content: str, source: str = "user_input", user_id: str = "default") -> str:
        ...
        # Push key concepts into Astra's mind (stored_knowledge) so learning is unified
        self._push_learning_to_mind(concepts, key_points, content, source)
    ...
    def _push_learning_to_mind(
        ...
        """Push key concepts/summaries into Astra's mind (stored_knowledge) so learning is unified."""
        try:
            mind_data = session.load()
            mind_data.setdefault("stored_knowledge", [])
            existing = set(
                (e.get("insight", e) if isinstance(e, dict) else e).strip().lower()
                for e in mind_data["stored_knowledge"]
            )
            summary = " ".join(key_points)[:200] if key_points else content[:200]
            added = 0
            for concept in concepts[:max_entries]:
                if not concept or len(concept) < 3:
                    continue
                entry = f"📖 **{concept}**: {summary}"
                key = entry.strip().lower()[:300]
            ...
            if added:
                session.maybe_save()
                print(f"[learning_service] Pushed {added} concept(s) to mind stored_knowledge.")
        except Exception as e:
            print(f"[learning_service] Failed to push learning to mind: {e}")
