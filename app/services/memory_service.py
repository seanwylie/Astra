...
class MemoryService:
    ...
    def get_memory_snippet(self, user_id: str = "default", max_items: int = 5) -> str:
        """Return a short string of key: value pairs for reply context (e.g. 'key1: value1; key2: value2')."""
        if user_id not in self.memories or not self.memories[user_id]:
            return "Nothing specific yet."
        items = list(self.memories[user_id].items())[:max_items]
        parts = [f"{k}: {v.get('value', v) if isinstance(v, dict) else v}" for k, v in items]
        return "; ".join(parts) if parts else "Nothing specific yet."
