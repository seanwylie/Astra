...
async def handle_on_ready(bot, channel_id: int):
    ...
    # Step 1.5: Set bot nickname to "Astra" in the server (if we have permission)
    try:
        guild = channel.guild if channel else None
        if guild and guild.me:
            current_nick = guild.me.display_name
            if current_nick != "Astra":
                await guild.me.edit(nick="Astra")
                print(f"✅ Set bot nickname to 'Astra' in {guild.name}")
    except discord.Forbidden:
        print("⚠️ No permission to change bot nickname")
    except Exception as e:
        print(f"⚠️ Failed to set bot nickname: {e}")
