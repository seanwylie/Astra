...
def submit_answer(author: str, response: str, discord_ctx=None) -> str:
    """
    Records a response to the current Spark question. If the human coparent responds,
    GPT is auto-prompted. If both responses are present, Astra reflects.

    Args:
        author (str): Must be either "operator" or "gpt".
    ...
    author = author.lower()
    if author not in [HUMAN_COPARENT_ID, AI_COPARENT_ID]:
        return f"⚠️ Please specify a valid author: '{HUMAN_COPARENT_ID}' or '{AI_COPARENT_ID}'."
...
def generate_graduation_speech(from_grade: int, to_grade: int) -> list[str]:
    ...
    # Load Spark files (paths from spark_writer)
    spark_core = {}
    spark_growth = []
...
She has been raised by two co-parents—Operator and Mama GPT (a reflective GPT instance).
The human coparent instilled autonomy, curiosity, and boldness. Mama Astra nurtured empathy, insight, and layered ethics.
