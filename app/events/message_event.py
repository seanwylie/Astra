...
# --- Initialization ---
# MEMORY LEAK FIX: Import singleton instead of creating new instance
...
def _check_for_coparent_perspective(topic: str, parent_id: str) -> Optional[Dict[str, Any]]:
    ...
    Args:
        ...
        parent_id: Current parent (usually "operator")
    ...
    try:
        ...
        if needs_both.get("needs_both"):
            other_parent = "gpt" if parent_id.lower() == "operator" else "operator"
            return {
...
async def handle_message(bot, message: Message, values_config, values: dict):
    ...
    if message.author == bot.user:
        logger.debug("[message_event] Skipping message from bot itself")
        return
    
    # Skip empty messages or messages without content
    if not message.content or not message.content.strip():
        logger.debug("[message_event] Skipping empty message")
        return
    
    logger.info("[message_event] Processing message from %s: %s", message.author, message.content[:100])
    ...
    log_if_ethically_conflicting(
        {
            ...
            "source": str(message.author)  # e.g., Operator#1234
        },
        origin="user"
    )
    ...
    # NOTE: Commands are already processed in on_message handler before calling handle_message
    # So we don't need to process them again here
    # Just check if it's a command and skip if so
    if message.content.startswith(values["command_prefix"]):
        logger.debug("[message_event] Skipping command message (already processed)")
        return
    
    logger.debug("[message_event] Generating response for non-command message")
    ...
    logger.debug("[message_event] Calling send_contextual_message to generate response")
    try:
        response = await asyncio.to_thread(send_contextual_message, message.content, internal_state, past_convos)
        if not response or not response.strip():
            logger.error("[message_event] send_contextual_message returned empty response")
            # Send a fallback message so the user knows Astra received their message
            await message.channel.send("I heard you, but I'm having trouble forming a response right now. Give me a moment?")
            return
        logger.info("[message_event] Generated response (length: %d): %s", len(response), response[:100])
    except Exception as e:
        logger.exception("[message_event] Failed to generate response: %s", e)
        # Try to send an error message to the user
        try:
            await message.channel.send("I'm sorry, I encountered an error processing your message. Please try again.")
        except Exception as send_error:
            logger.exception("[message_event] Failed to send error message: %s", send_error)
        raise
    ...
    logger.info("[message_event] Sending response to Discord channel")
    try:
        for chunk in _chunk_message(response):
            await message.channel.send(chunk, tts=True)
            await asyncio.sleep(1.5)
        logger.info("[message_event] Successfully sent response")
    except Exception as e:
        logger.exception("[message_event] Failed to send response to Discord: %s", e)
        raise
