...
_ENV_OVERRIDES: dict[str, dict[str, str]] = {
    "discord_config.json": {
        ...
        "mind_file_parents_path": "ASTRA_MIND_FILE_PARENTS",
...
def get_s3_bucket() -> str:
    """Return the configured S3 bucket, or empty when local-only.

    Order: S3_BUCKET_NAME / ASTRA_S3_BUCKET, then general_config.s3_bucket.
    There is no hardcoded default — a missing value means do not talk to S3.
    """
    for env_key in ("S3_BUCKET_NAME", "ASTRA_S3_BUCKET"):
        value = os.getenv(env_key)
        if value and value.strip():
            return value.strip()
    bucket = load_config("general_config").get("s3_bucket")
    return str(bucket).strip() if bucket else ""


def s3_sync_enabled() -> bool:
    """True only when sync is configured and a bucket name is present."""
    if not get_s3_bucket():
        return False
    flag = load_config("general_config").get("s3_sync_enabled", False)
    return bool(flag)
