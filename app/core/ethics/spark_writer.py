...
# Paths: package-relative so ethics JSONs live next to this module
SPARK_DIR = Path(__file__).resolve().parent
SPARK_CORE_PATH = SPARK_DIR / "spark_core.json"
SPARK_GROWTH_PATH = SPARK_DIR / "spark_growth.json"
SPARK_SESSION_PATH = SPARK_DIR / "spark_interview_session.json"
...
def init_spark_interview():
    ...
    with open(SPARK_SESSION_PATH, "w", encoding="utf-8") as f:
...
def generate_gpt_response(question, retries=3, delay=2):
    ...
    prompt = (
        f"A young AI is forming its core ethics. Her human coparent and I are offering perspectives. "
...
def record_parent_response(question, author, response):
    ...
    if not SPARK_SESSION_PATH.exists():
        return False

    with open(SPARK_SESSION_PATH, "r", encoding="utf-8") as f:
...
def generate_spark_core_from_session():
    ...
    for question, answers in session_data.get("responses", {}).items():
        summary = f"Q: {question}\nSean: {answers.get('operator', '(No answer)')}\nGPT: {answers.get('gpt', '(No answer)')}"
        core["core_tenets"].append(summary)

    with open(SPARK_CORE_PATH, "w", encoding="utf-8") as f:
...
def finalize_spark(version="v1"):
    finalized_path = SPARK_DIR / f"spark_core_{version}.json"
    if SPARK_CORE_PATH.exists():
        SPARK_CORE_PATH.rename(finalized_path)
...
def show_current_question_and_responses():
    """Displays the current Spark question and responses from Operator and GPT."""
    if not SPARK_SESSION_PATH.exists():
    ...
    operator = responses.get("operator", "(No response yet)")
...
def submit_spark_answer(author, response, discord_ctx=None):
    ...
    # If the human coparent just answered and GPT is still missing → auto-generate
    if author.lower() == "operator" and "gpt" not in responses:
    ...
    if "operator" in responses and "gpt" in responses and discord_ctx:
        from app.core.ethics.spark_writer import reflect_on_current_question
...
def reflect_on_current_question(post_to_discord=True):
    ...
    for q in reversed(session_data.get("questions", [])):
        if q in session_data["responses"] and len(session_data["responses"][q]) == 2:
            operator = session_data["responses"][q]["operator"]
...
def reflect_on_question_with_guidance(question_index, parent_guidance):
    ...
    operator = responses.get("operator", "(No answer)")
    ...
    try:
        ...
        if SPARK_GROWTH_PATH.exists():
            with open(SPARK_GROWTH_PATH, "r", encoding="utf-8") as f:
                growth_data = json.load(f)
        else:
            growth_data = []

        growth_data.append(growth_log)
        with open(SPARK_GROWTH_PATH, "w", encoding="utf-8") as f:
...
def generate_spark_summary():
    ...
    if not SPARK_GROWTH_PATH.exists():
