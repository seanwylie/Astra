...
# MEMORY LEAK FIX: Add hard limit to prevent unbounded growth
_trigger_log = []
_MAX_TRIGGER_LOG_ENTRIES = 10000  # Hard limit to prevent memory leak
...
def _apply_trigger_rate_limit(emotion_name, trigger_event, change, config):
    ...
    global _trigger_log
    # MEMORY LEAK FIX: Clean up old entries and enforce hard limit
    _trigger_log = [(e, t, ts, d) for (e, t, ts, d) in _trigger_log if ts >= cutoff]
    
    # Enforce hard limit - keep most recent entries
    if len(_trigger_log) > _MAX_TRIGGER_LOG_ENTRIES:
        _trigger_log = _trigger_log[-_MAX_TRIGGER_LOG_ENTRIES:]
        logger.debug("Trigger log exceeded limit, trimmed to %s entries", _MAX_TRIGGER_LOG_ENTRIES)
