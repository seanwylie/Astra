...
async def ask_dinner_question(bot, channel, topic):
    ...
    await channel.send("👨‍👧 Operator, what are your thoughts?")
...
def save_current_dinner_timestamp(timestamp):
    ...
    try:
        ...
        _s3_client().put_object(Bucket=S3_BUCKET_NAME, Key=DINNER_CURRENT_KEY, Body=body)
    except Exception as e:
...
def load_current_dinner_timestamp():
    ...
    if not S3_BUCKET_NAME:
        return None
    try:
        response = _s3_client().get_object(Bucket=S3_BUCKET_NAME, Key=DINNER_CURRENT_KEY)
