...
from app.core.ethics import spark_writer
from app.core.ethics.spark_writer import SPARK_CORE_PATH, SPARK_GROWTH_PATH
...
from app.interfaces.influence import save_mind
from app.core.message_generator import MessageGenerator, handle_openai_fallback
...
from app.core.emotions.emotion_engine import get_top_emotions, trigger_emotion, decay_all_emotions
...
async def spark_reflect(ctx, question_number: int, *, guidance: str):
    ...
    from app.core.ethics.spark_writer import reflect_on_question_with_guidance
...
async def spark_review(ctx):
    ...
    from app.core.ethics.spark_writer import generate_spark_summary
...
async def spark_graduation(ctx, from_grade: int, to_grade: int):
    ...
    try:
        ...
        spark_core = {}
        if SPARK_CORE_PATH.exists():
            with open(SPARK_CORE_PATH, encoding="utf-8") as f:
                spark_core = json.load(f)

        # Load Spark Growth (recent reflections and adjustments)
        spark_growth = []
        if SPARK_GROWTH_PATH.exists():
            with open(SPARK_GROWTH_PATH, encoding="utf-8") as f:
...
async def on_ready():
    ...
    from app.core.messaging.message_bus import describe_emotional_state, get_dominant_emotion
    from app.core.emotions.emotion_engine import load_emotion_state
...
async def dinner_summary(ctx):
    ...
    from app.core.dinner.dinner_journal import summarize_dinner_journal
    summary = summarize_dinner_journal()
    await ctx.send(summary)

@bot.command(name="dinner_answer")
async def handle_user_dinner_answer(ctx, *, response):
    """Answer Astra during Dinner time questions. GPT responds after you, then Astra shares her reflection."""
    from app.core.dinner.dinner_journal import load_dinner_journal, mark_dinner_responded
    from app.core.astra_schedule.dinner import try_resolve_dinner_topic, get_current_dinner_timestamp, get_gpt_dinner_response
...
async def resolve_dinner_now(ctx):
    ...
    from app.core.dinner.dinner_journal import get_resolvable_dinner_topics, resolve_dinner_topic
    from app.core.astra_schedule.dinner import astra_reason  # ✅ NEW
...
async def run_dream_time(ctx):
    ...
    from app.core.astra_schedule.dream import process_dream_seed  # lazy import to avoid circular deps
