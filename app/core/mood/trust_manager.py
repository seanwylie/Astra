...
class TrustManager:
    ...
    def _apply_trust_decay_if_due(self):
        ...
        decay_rate = self.config.get("trust_decay_rate", 0.0)
        if decay_rate <= 0:
            return
        today = time.strftime("%Y-%m-%d")
        last_decay = self.mind_data.get("last_trust_decay_date")
        if last_decay == today:
            return
        try:
            ...
            if days < 1:
                return
            default = self.config["default_entity_trust"]
            min_trust, max_trust = self.config["trust_bounds"]
            for entity in list(self.entity_trust.keys()):
                trust = self.entity_trust[entity]
                for _ in range(days):
                    trust = trust + (default - trust) * decay_rate
                    trust = max(min_trust, min(trust, max_trust))
                self.entity_trust[entity] = round(trust, 4)
            self.mind_data["last_trust_decay_date"] = today
            self.save_trust_state()
        except Exception as e:
            print(f"[trust_manager] trust decay failed: {e}")
    ...
    def modify_trust(self, entity, change, reason=""):
        ...
        # ❌ **Trust loss is harsher**
            ...
            # 🚨 **Prevent trust from instantly dropping too low** (scale 0-1)
            min_trust, _ = self.config["trust_bounds"]
            if trust + change < min_trust + 0.2:
        ...
        # Personality: trust damage shifts traits (plan: wire update_personality to trust)
        if change < 0:
            try:
                from app.core.personality.personality_manager import update_personality
                magnitude = min(1.0, abs(change) * 2.0)
                update_personality("betrayed_trust", magnitude)
            except Exception as e:
                print(f"[trust_manager] update_personality failed: {e}")
            # Outcome-based emotion: trust loss triggers anger/resentment (plan: richer emotion triggers)
            try:
                from app.core.emotions.emotion_engine import trigger_emotion
                trigger_emotion("anger", "betrayal")
                trigger_emotion("resentment", "betrayal")
            except Exception as e:
                print(f"[trust_manager] trigger_emotion failed: {e}")
        else:
            # Outcome-based emotion: trust gain triggers love/hope (plan: richer emotion triggers)
            try:
                from app.core.emotions.emotion_engine import trigger_emotion
                trigger_emotion("love", "positive_reinforcement")
