...
from app.config.loader import load_config, get_s3_bucket
...
class SelfModel:
    ...
    def _load_self_model(self) -> None:
        ...
        try:
            ...
            # MEMORY LEAK FIX: Only load recent snapshots/changes to limit memory usage
            MAX_HISTORICAL_SNAPSHOTS = 50
            MAX_CHANGES = 100
            MAX_SURPRISE_LOG = 50
            
            all_snapshots = [SelfSnapshot.from_dict(s) for s in data.get("historical_snapshots", [])]
            self.historical_snapshots = all_snapshots[-MAX_HISTORICAL_SNAPSHOTS:] if len(all_snapshots) > MAX_HISTORICAL_SNAPSHOTS else all_snapshots
            
            all_changes = [SelfChange.from_dict(c) for c in data.get("changes", [])]
            self.changes = all_changes[-MAX_CHANGES:] if len(all_changes) > MAX_CHANGES else all_changes
            
            all_surprise_log = data.get("surprise_log", [])
            self.surprise_log = all_surprise_log[-MAX_SURPRISE_LOG:] if len(all_surprise_log) > MAX_SURPRISE_LOG else all_surprise_log
    ...
    def take_snapshot(self) -> SelfSnapshot:
        ...
        # MEMORY LEAK FIX: Enforce limit before appending
        MAX_HISTORICAL_SNAPSHOTS = 50
        self.historical_snapshots.append(snapshot)
        if len(self.historical_snapshots) > MAX_HISTORICAL_SNAPSHOTS:
            self.historical_snapshots = self.historical_snapshots[-MAX_HISTORICAL_SNAPSHOTS:]
            logger.debug("Trimmed historical_snapshots to %s entries", MAX_HISTORICAL_SNAPSHOTS)
        self._save_self_model()
    ...
    def predict_own_response(self, situation: str) -> Dict[str, Any]:
        ...
        if any(word in situation_lower for word in ["love", "care", "family", "operator"]):
