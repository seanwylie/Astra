...
def track_question_patterns(mind_data):
    ...
    for q in self_questions:
        if isinstance(q, dict):
            question_val = q.get("question") or ""
            # Handle case where question might be a dict
            if isinstance(question_val, dict):
                norm = str(question_val).strip().lower()
            else:
                norm = str(question_val).strip().lower()
        else:
            norm = str(q or "").strip().lower()
