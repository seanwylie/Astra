...
def search_wikipedia(query):
    try:
        ...
        mind_data = load_mind()
        if any(best_match in entry for entry in mind_data["stored_knowledge"]):
            return f"I already know about {best_match}. Maybe I should connect it to my existing thoughts?"

        new_knowledge = f"From Wikipedia ({best_match}): {summary}"
        mind_data["stored_knowledge"].append(new_knowledge)
        mind_data["self_questions"].append({"question": f"What implications does '{best_match}' have on my understanding?"})
